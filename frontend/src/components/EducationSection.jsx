...
/**
 * @brief Education history section.
 * 
 * @param {Array} items List of education records.
 * @param {Function} onAdd Callback to add a new record.
 * @param {Function} onDelete Callback to delete a record by ID.
 * @param {Function} onEdit Callback to edit a specific record.
                    ...
                    <div className="space-y-6">
                        {items.map((entry, idx) => (
                            <article key={entry.id || idx} className="relative rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md sm:pl-12">
                                ...
                                {/* Action Buttons (for Dashboard use cases) */}
                                {/* {(onEdit || onDelete) && (
